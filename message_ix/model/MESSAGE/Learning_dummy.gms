...
*0.91
  ...
  nbr_unit_ref(newtec)                   'initial number of unit'                / wind_ppl      42208     /
...
CAPEX_ESTIMATE(newtec,year)..  CAPEX_TEC(newtec,year)*cap_new2(newtec,year) =g= sum(size,inv_cost_ref(newtec)
                                              ...
                                              * [(((sum((size2,year2)$(ord(year2) le ord(year) and ord(year2) gt (ord(year)-1)), NBR_UNIT(newtec,size2,year2))+nbr_unit_ref(newtec))/nbr_unit_ref(newtec))**(-b(newtec)))]
